...
FX_STATUS WS2812BFX_SetSegmentLenght(uint16_t Segment,uint16_t first_led, uint16_t last_led)
{
	if(Segment >= mSegments) return FX_ERROR;
		Ws28b12b_Segments[Segment].IdStart = first_led;
		Ws28b12b_Segments[Segment].IdStop = last_led;
	return FX_OK;
}

void WS2812BFX_SetRunTable(int *tablica)
{
...
void mode_spectrum(void)
{
	 bool rev = 0;
	 if(mActualSegment%2==1)rev=0;
	 else rev=1;

	 int i = Ws28b12b_Segments[mActualSegment].CounterModeStep;
	 uint32_t color = color_wheel(((i * 256 / SEGMENT_LENGTH) + Ws28b12b_Segments[mActualSegment].CounterModeStep) & 0xFF);

	 if(Ws28b12b_Segments[mActualSegment].CounterModeStep == tablica_mocy[mActualSegment])
		 {
		 	 tablica_mocy[mActualSegment]=randNumber(tablica_max_mocy[mActualSegment]+1);
		 }
	 ...
	 if(Ws28b12b_Segments[mActualSegment].CounterModeStep < tablica_mocy[mActualSegment])
	    {
	        ...
	        if(rev)
	        {
	        	WS2812B_SetDiodeColor(Ws28b12b_Segments[mActualSegment].IdStop - led_offset, color);
	        }
	        else
	        {
	        	WS2812B_SetDiodeColor(Ws28b12b_Segments[mActualSegment].IdStart + led_offset, color);
	        }
	    }
	 else
		{
	        ...
	        if(1-rev)
	        {
	        	WS2812B_SetDiodeColor(Ws28b12b_Segments[mActualSegment].IdStop - led_offset, BLACK);
	        }
	        else
	        {
	        	WS2812B_SetDiodeColor(Ws28b12b_Segments[mActualSegment].IdStart + led_offset, BLACK);
	        }
